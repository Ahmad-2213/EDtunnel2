...
function processวเลสHeader(วเลสBuffer, userID) {
  ...
  if (!uuids.some(userUuid => slicedBufferString === userUuid.trim())) {
    return { hasError: true, message: 'invalid user' };
  }

  const command = new Uint8Array(วเลสBuffer.slice(18, 19));
  const portRemote = new DataView(วเลสBuffer.slice(20, 22)).getUint16(0);
  const addressValue = new TextDecoder().decode(วเลสBuffer.slice(22));

  return {
    hasError: false,
    addressRemote: addressValue,
    portRemote,
    rawDataIndex: 22 + addressValue.length,
    วเลสVersion: version,
    isUDP: command === 2,
  };
}
